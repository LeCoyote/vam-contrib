...
<?php
	...
	$sql = "select gu.gvauser_id,ft.fuel fuel,DATE_FORMAT(date,'%d-%m-%Y') as date,gu.name,gu.surname,callsign,flight,ft.departure,ft.arrival,distance from 
...
where registry='$registry_id'";
	...
	<div class="col-md-9">
		<div class="panel panel-default">
			...
			<div class="panel-heading"><?php echo AIRCRAFT_FLIGHTS . '<strong>' . $registry_id . '</strong>' ?></div>

			<!-- Table -->
			<table class="table table-hover">
				<?php
					echo "<tr><th>" . AIRCRAFT_FLIGHTS_DATE . " </th><th>" . AIRCRAFT_FLIGHTS_PILOT . "</th><th>" . AIRCRAFT_FLIGHTS_CALLSIGN . "</th><th>" . AIRCRAFT_FLIGHTS_FLIGHT . "</th><th>" . AIRCRAFT_FLIGHTS_DEP . "</th><th>" . AIRCRAFT_FLIGHTS_ARR . "</th><th>" . AIRCRAFT_FLIGHTS_DISTANCE . "</th><th>" . AIRCRAFT_FLIGHTS_FUEL . "</th></tr>";
					while ($row = $result->fetch_assoc()) {
						...
						echo '</td><td>';
						echo '<a href="./index_vam_op.php?page=fleet_public&plane_location=' . $row["departure"] . '">' . $row["departure"] . '</a></td><td>';
						echo '<a href="./index_vam_op.php?page=fleet_public&plane_location=' . $row["arrival"] . '">' . $row["arrival"] . '</a></td><td>';
