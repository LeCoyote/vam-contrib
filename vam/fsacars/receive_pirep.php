	...
	function InsertReportIntoDB($pirep_array)
	{
		...
		and date='$dateflight' and time='$timeflight'";
			...
			} else
			{
				...
				//$sqlflight = 'select * from gvausers gu, routes r where gu.gvauser_id=' . $pilot_id . ' and gu.route_id <> 0 and r.departure="' . $origin_id . '" and r.arrival="' . $destination_id . '" and gu.route_id=r.route_id;';
				...
				if ($charter == 0 || $duplicated==1 ) {
					...
					*/
					...
					//$sqlfleet = 'select * from  fleets  where fleet_id = (select fleet_id from  reserves where gvauser_id=' . $pilot_id . ');';
					...
					if ($estado < 0) {
						$query1 = "insert into hangar (registry,gvauser_id,fleet_id,departure,location,date_in,date_out,reason) values ('$matricula',$pilot_id,$avion,'$origen','$location',CURDATE(),ADDDATE(CURDATE(),$hangar_crash_days),'Accidente')";
						if (!$result = $db->query($query1)) {
							die('There was an error running the query  [' . $db->error . ']');
							return 0;
						}
						...
						$query1 = "insert into bank (gvauser_id, date,quantity,jump) values ($pilot_id,now(),-$pilot_crash_penalty,'Accidente $matricula')";
						if (!$result = $db->query($query1)) {
							die('There was an error running the query  [' . $db->error . ']');
							return 0;
						}
