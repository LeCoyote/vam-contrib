			...
			<div>
				<div>
					...
					<?php
						...
						// $sql= "select valid as status,pirep_id as flight,from_airport departure, to_airport arrival ,date,distance,duration,charter, 'pirep' as type from pireps where gvauser_id=$id UNION
						//select validated as status,pirepfsfk_id as flight, SUBSTRING(OriginAirport,1,4) departure, SUBSTRING(DestinationAirport,1,4) arrival , CreatedOn as date, DistanceFlight as distance, FlightTime as duration, charter , 'keeper' as type from pirepfsfk where gvauser_id=$id order by date desc";
